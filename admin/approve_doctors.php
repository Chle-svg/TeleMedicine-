...
if ($search !== '') {
    $stmt = $conn->prepare("SELECT COUNT(*) AS total FROM doctor_applications INNER JOIN users ON doctor_applications.user_id = users.id WHERE doctor_applications.status = 'Approve' $search_sql");
    $stmt->bind_param("s", $search_param);
    $stmt->execute();
    $totalResult = $stmt->get_result();
} else {
    $totalResult = mysqli_query($conn, "SELECT COUNT(*) AS total FROM doctor_applications WHERE status = 'Approve'");
}
...
if ($search !== '') {
    $stmt = $conn->prepare("
        ...
        WHERE doctor_applications.status = 'Approve' $search_sql
    ...
    $query = "
        ...
        FROM doctor_applications
        INNER JOIN users ON doctor_applications.user_id = users.id
        WHERE doctor_applications.status = 'Approve'
        ORDER BY doctor_applications.id DESC
...
<html>
<head>
    <title>Accepted Doctor Applications - Admin Panel</title>
...
<div class="admin-container">
    ...
    <div class="main-content">
        ...
        <div class="content-area">
            <h2><i class="fas fa-user-check"></i> Accepted Doctor Applications</h2>
            <p>View all Accepted doctor applications in the system.</p>
            ...
            <table>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="7" style="text-align:center;">No Accepted applications found.</td>
                    </tr>
                <?php endif; ?>
                </tbody>
            </table>
